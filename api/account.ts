...
export const getAccount =
  ...
  async (args: { npub?: string; username?: string }) => {
    if (args.npub) {
...
export const login =
  (urlArg: URL) => async (args: { username: string; password: string }) => {
    const url = copyURL(urlArg);
    url.pathname = `/login`;
    const response = await safeFetch(url, {
      method: "POST",
    ...
    if (result instanceof ApiError) {
      if (result.status == 404) {
        ...
        result.status == 401 &&
        result.message == "Account is blacklisted"
      ) {
...
export const createAccount =
  (urlArg: URL) =>
  async (args: { username: string; password: string; email: string }) => {
    const url = copyURL(urlArg);
...
export const initiatePasswordReset =
  (urlArg: URL) => async (args: { username: string }) => {
    const url = copyURL(urlArg);
...
export const checkUsernameAvailabilityInSatlantis =
  (urlArg: URL) => async (args: { name: string }) => {
    const url = copyURL(urlArg);
...
export const resetPassword =
  (urlArg: URL) => async (args: { token: string; password: string }) => {
    const url = copyURL(urlArg);
...
export const getAccountCalendarEvents =
  (urlArg: URL) => async (args: GetAccountCalendarEventsArgs) => {
    const url = copyURL(urlArg);
...
export const getEventsByAccount =
  ...
  async (args: GetEventsByAccountArgs) => {
    const url = copyURL(urlArg);
    url.pathname = `/account/${args.npub}/events`;
    const period = args.period ?? CalendarEventPeriod.Upcoming;
    url.searchParams.set("period", period.toString());
...
export const verifyOTP =
  (urlArg: URL) => async (args: { token: string; otp: string }) => {
    const url = copyURL(urlArg);
